...
bool HistoryStore::init()
{
    ...
    const QString dbPath = dataDir + QLatin1String("/history.db");
...
bool HistoryStore::ensureSchema()
{
    ...
    // 获取表信息
    query.exec("PRAGMA table_info(clipboard_history)");
    ...
    else
    {
        ...
        {
            query.exec("ALTER TABLE clipboard_history ADD COLUMN item_id TEXT");
        }
        if (!existingColumns.contains("image_path"))
        {
            query.exec("ALTER TABLE clipboard_history ADD COLUMN image_path TEXT");
        }
        if (!existingColumns.contains("image_thumb"))
        {
            query.exec("ALTER TABLE clipboard_history ADD COLUMN image_thumb TEXT");
        }
        if (!existingColumns.contains("hash"))
        {
            query.exec("ALTER TABLE clipboard_history ADD COLUMN hash TEXT");
        }
        if (!existingColumns.contains("pinned"))
        {
            query.exec("ALTER TABLE clipboard_history ADD COLUMN pinned INTEGER DEFAULT 0");
        }
    }

    // 创建索引
    query.exec("CREATE INDEX IF NOT EXISTS idx_clipboard_history_ts ON clipboard_history(timestamp DESC)");
    query.exec("CREATE INDEX IF NOT EXISTS idx_clipboard_history_hash ON clipboard_history(hash)");
    query.exec("CREATE INDEX IF NOT EXISTS idx_clipboard_history_pinned ON clipboard_history(pinned)");
...
bool HistoryStore::hasHash(const QString &hash) const
{
    ...
    QSqlQuery query(m_db);
    query.prepare("SELECT id FROM clipboard_history WHERE hash = ? LIMIT 1");
...
bool HistoryStore::updateTimestampByHash(const QString &hash)
{
    ...
    QSqlQuery query(m_db);
    query.prepare("UPDATE clipboard_history SET timestamp = ? WHERE hash = ?");
...
QVector<ClipboardItem> HistoryStore::loadLatest(int limit)
{
    ...
    QSqlQuery query(m_db);
    query.prepare("SELECT id, item_id, type, content, image_path, image_thumb, hash, timestamp, pinned FROM clipboard_history ORDER BY pinned DESC, timestamp DESC LIMIT ?");
...
bool HistoryStore::addItem(const ClipboardItem &item)
{
    ...
    QSqlQuery query(m_db);
    query.prepare(
        "INSERT INTO clipboard_history (id, item_id, type, content, image_path, image_thumb, hash, timestamp, pinned) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
bool HistoryStore::updateTextItem(qint64 dbId, const QString &content, const QString &hash)
{
    ...
    QSqlQuery query(m_db);
    query.prepare("UPDATE clipboard_history SET content = ?, hash = ?, timestamp = ? WHERE id = ?");
...
bool HistoryStore::setPinned(qint64 dbId, bool pinned)
{
    ...
    QSqlQuery query(m_db);
    query.prepare("UPDATE clipboard_history SET pinned = ? WHERE id = ?");
...
bool HistoryStore::deleteById(qint64 dbId)
{
    ...
    // 获取要删除的项目（包括图片文件）
    QSqlQuery query(m_db);
    query.prepare("SELECT image_path, image_thumb FROM clipboard_history WHERE id = ?");
    query.addBindValue(dbId);
